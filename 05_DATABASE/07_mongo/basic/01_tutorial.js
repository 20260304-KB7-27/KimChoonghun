use tutorial // 데이터베이스를 tutorial로 선정하세요

db.users.insertOne({ username : "smith"}) // users 컬렉션에 username이 smith인 문서를 저장하세요.
db.users.insertOne({ username : "jones"}) // users 컬렉션에 username이 jones인 문서를 저장하세요.
db.users.find() // 앞에서 저장한 모든 문서를 출력하세요
db.users.findOne() // 앞에서 저장한 문서 중 하나 만 출력하세요
...
page_number = 6
db.product.find().skip((page_number-1)*10)
    .limit(10)
    .sort({num: -1})
...
// product 컬렉션에서 name에 '스마트폰 10', '스마트폰 100', '스마트폰 1000' 중에 하나이면 출력하세요
db.product.find(
    {
        name: {
            $in: [
